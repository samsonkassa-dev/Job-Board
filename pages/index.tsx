...
const Home: NextPage = () => {
  ...
  return (
    <>
      <div className={`flex bg-white max-w-5xl mx-14 sm:mx-auto min-h-[72px] box-border pl-12 -mt-5 py-4 rounded-xl ${!isSearch && "hidden"}`}>
      ...
      {filterData.map((advert) => (
        <div className={`flex rounded-lg flex-col sm:flex-row pl-3.5 pt-3 pb-3 border-2 bg-white ${advert.featured && "border-l-8 border-l-[#5CA5A5]"}`} key={advert.id}>
          ...
          <div className="flex flex-wrap gap-4 mr-10 sm:content-center sm:justify-end mt-4">
            {advert.tags.map(
              (tag) => <span className="spanTag" key={tag} >{tag}</span>)}
          </div>
        </div>
      ))}
    </div>
